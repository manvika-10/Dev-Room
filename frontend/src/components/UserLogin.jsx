...
let UserLogin = () => {
  ...
  let submitLogin = async(event) => {
    ...
    if (user.email !== "" && user.password !== "") {
     ...
     const {status,data}=await axios.post('https://devconnect-backend-nno6.onrender.com/api/users/login',{email,password},{
        headers:{
      ...
      if(status==201){
        Swal.fire("Invalid credentials","", "error");
         ...
         localStorage.setItem("devconnect", data.token);
         navigate("/");
      }
